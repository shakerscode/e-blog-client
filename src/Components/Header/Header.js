...
const Header = () => {
  ...
  return (
    <header className="shadow sticky top-0 bg-white z-50">
      <div
        className={`flex justify-between items-center md:px-14 px-3 md:py-4 py-1 h-20 md:h-auto sticky top-0  ${
      ...
      >
        ...
        <h1 className="md:text-4xl text-2xl font-bold text-primary">
          e<span className="text-secondary">Blog</span>
        </h1>
        ...
        <div className="flex items-center gap-5  text-center ">
          <ul
            className={`flex flex-col md:flex-row md:justify-end items-center md:gap-10 bg-white md:bg-none w-full md:w-auto shadow-lg md:shadow-none py-4 gap-3 z-20 md:z-auto absolute md:static transition-all duration-500 ease-in-out left-0 ${
              open
                ? "top-20 opacity-100"
                : "top-[-250px] left-0 opacity-0 md:opacity-100"
            }`}
          >
            {menus.map((menu, i) => (
              <li
                key={i}
                className="text-sm md:text-lg cursor-pointer hover:text-secondary transition-all ease-in duration-150 text-[20px]"
              >
                <a
                  target={`${
                    menu?.m === "Our Shop" ||
